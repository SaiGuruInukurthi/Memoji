...
function checkMorphTargets() {
    if (app && app.avatar) {
        return app.analyzeMorphTargets();
    } else {
...
function forceAvatarActivation() {
    if (app && app.avatar) {
        ...
        if (app.scene) {
            ...
            app.scene.meshes.forEach((mesh, index) => {
        ...
        if (app.avatar.allHeadParts) {
            const newMorphTargets = app.findMorphTargets(app.avatar.allHeadParts);
            console.log('🎭 Rebuilt morph targets:', Object.keys(newMorphTargets));
            app.avatar.morphTargets = newMorphTargets;
        }
...
function syncMorphTargetsAcrossAllMeshes() {
    if (app && app.scene) {
        ...
        const mouthOpenValue = app.calculateMouthOpenness();
        const smileValue = app.expressions?.happy || 0;
...
function testExtremeValues() {
    if (app && app.avatar && app.avatar.morphTargets) {
        console.log('💥 === EXTREME VALUE TEST ===');
        
        const targets = Object.keys(app.avatar.morphTargets);
        targets.forEach(targetName => {
            const targetData = app.avatar.morphTargets[targetName];
            if (targetData && targetData.target) {
...
function checkAvatarGeometry() {
    if (app && app.avatar) {
        ...
        if (app.avatar.allHeadParts) {
            app.avatar.allHeadParts.forEach(mesh => {
        ...
        console.log(`Scene render count: ${app.scene.getRenderedMeshes().length}`);
        console.log(`Active meshes: ${app.scene.getActiveMeshes().length}`);
    }
}

function checkEyeTargets() {
    if (app && app.avatar && app.avatar.morphTargets) {
        const allTargets = Object.keys(app.avatar.morphTargets);
        const eyeTargets = allTargets.filter(key => 
            key.toLowerCase().includes('eye') || 
            key.toLowerCase().includes('blink') ||
            key.toLowerCase().includes('lid')
        ...
        if (eyeTargets.length === 0) {
            ...
            eyeTargets.forEach(target => {
                const morphTarget = app.avatar.morphTargets[target];
                if (morphTarget && morphTarget.target) {
...
function testMouth() {
    if (app) {
        ...
        app.forceMouthTest(0.8, 0.6); // Test with strong values
    } else {
        console.log('❌ App not initialized yet');
    }
}
...
function debugMouth() {
    if (app) {
        ...
        if (app.landmarks) {
            const mouth = app.landmarks.getMouth();
            console.log('👄 Mouth landmarks count:', mouth.length);
            
            const openness = app.calculateMouthOpenness();
            console.log('👄 Current mouth openness:', (openness * 100).toFixed(1) + '%');
            
            if (app.expressions) {
                console.log('😊 Happy expression:', (app.expressions.happy * 100).toFixed(1) + '%');
                console.log('😮 Surprised expression:', (app.expressions.surprised * 100).toFixed(1) + '%');
                console.log('😢 Sad expression:', (app.expressions.sad * 100).toFixed(1) + '%');
                console.log('😠 Angry expression:', (app.expressions.angry * 100).toFixed(1) + '%');
            }
        } else {
            console.log('❌ No landmarks available');
        }
        
        if (app.avatar && app.avatar.morphTargets) {
            const mouthTargets = Object.keys(app.avatar.morphTargets).filter(key => 
                key.toLowerCase().includes('mouth') || 
                key.toLowerCase().includes('jaw') ||
                key.toLowerCase().includes('smile')
            );
